...
sed -i 's/^ro\.build\.version\.release=.*/ro.build.version.release=4.4-revolt/g' ${ANDROID_HOME}/platforms/android-${SDK_VER}-${CUSTOM_NAME}/build.prop
...
sed -i 's/Pkg.Desc=/Pkg.Desc=REVOLT /' ${ANDROID_HOME}/platforms/android-${SDK_VER}-${CUSTOM_NAME}/source.properties
